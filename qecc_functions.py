...
def syndrome_table_t_errors(N,P,codewords,errors_literal,affected_qubits):
...
def common_stabilizers(N,codewords,stab_set):
...
def syndrome_operators(N,codewords,stab_set):
    ...
    #computing the subset of commuting stabilizers
    common_commuting_stabilizers = []
    
    for i in range(len(common_stabilizers)):
        
        commutation_test = 0
        
        for j in range(i+1,len(common_stabilizers)):
            if uf.pauli_commutation(common_stabilizers[i],common_stabilizers[j]) == -1:
                commutation_test = 1
                break
        if commutation_test == 0:
            common_commuting_stabilizers.append(common_stabilizers[i])
        
    
    notas = np.zeros(len(common_commuting_stabilizers))
    
    for i in range(len(common_commuting_stabilizers)):
        
        len_x = len(uf.find_literal("X",np.array(common_commuting_stabilizers[i])))
        len_y = len(uf.find_literal("Y",np.array(common_commuting_stabilizers[i])))
        len_z = len(uf.find_literal("Z",np.array(common_commuting_stabilizers[i])))
    ...
    for i in range(len(common_commuting_stabilizers)):
        ...
        #only X
        if len(uf.find_literal("Y",np.array(common_commuting_stabilizers[i]))) == 0 and len(uf.find_literal("Z",np.array(common_commuting_stabilizers[i]))) == 0:
            
            stab_only_X.append([common_commuting_stabilizers[i],len(uf.find_literal("X",np.array(common_commuting_stabilizers[i])))])
            set_X.append(tuple(uf.find_literal("X",np.array(common_commuting_stabilizers[i]))))
            
        #only Y 
        elif len(uf.find_literal("X",np.array(common_commuting_stabilizers[i]))) == 0 and len(uf.find_literal("Z",np.array(common_commuting_stabilizers[i]))) == 0:
            
            stab_only_Y.append([common_commuting_stabilizers[i],len(uf.find_literal("Y",np.array(common_commuting_stabilizers[i])))])
            set_Y.append(tuple(uf.find_literal("Y",np.array(common_commuting_stabilizers[i]))))
            
            
        #only Z
        elif len(uf.find_literal("Y",np.array(common_commuting_stabilizers[i]))) == 0 and len(uf.find_literal("X",np.array(common_commuting_stabilizers[i]))) == 0:
            
            stab_only_Z.append([common_commuting_stabilizers[i],len(uf.find_literal("Z",np.array(common_commuting_stabilizers[i])))])
            set_Z.append(tuple(uf.find_literal("Z",np.array(common_commuting_stabilizers[i]))))
            
        else:
            
            stab_XYZ.append([common_commuting_stabilizers[i],len(uf.find_literal("X",np.array(common_commuting_stabilizers[i])))+len(uf.find_literal("Y",np.array(common_commuting_stabilizers[i])))+len(uf.find_literal("Z",np.array(common_commuting_stabilizers[i])))])
...
def correctability_degree(N,codewords,t,errors_literal,affected_qubits,stab_set):
    
    P, P_bit,topo_degree,css_group_degree, css_degre = syndrome_operators(N,codewords,stab_set)
    common_stab = common_stabilizers(N,codewords,stab_set)
    
    s_table = syndrome_table_t_errors(N,P,codewords,errors_literal,affected_qubits)
